...
La forma recomendada usa el `compose.yaml` de `limesurvey-dictionary-api`. **La revisión de desarrollo actual de esa API depende de archivos npm locales todavía sin publicar.** Antes de ofrecer un despliegue independiente limpio deben resolverse esas dependencias; consulte su README. FastAPI incluye sus artefactos de contrato generados y el validador local:
...
`verify_limesurvey_connection.py` prueba con datos reales: login, listado, carga de encuesta y
...
La exportación de Citric llega completa antes de comprobar su tamaño; no es una
descarga incremental desde LimeSurvey. Para volúmenes grandes, acote el intervalo
con `fromResponseId` y `toResponseId`. Los límites de tiempo y tamaño rechazan
exportaciones excesivas, pero no garantizan un consumo de memoria constante.
